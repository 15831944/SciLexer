...
class LexState : public LexInterface {
	//=================inc-new-flag================
	const Lexilla::LexerModule* lexCurrent;
	void SetLexerModule(const Lexilla::LexerModule* lex);
...
//=================inc-new-flag================
void LexState::SetLexerModule(const Lexilla::LexerModule* lex) {
	if (lex != lexCurrent) {
		if (instance) {
			instance->Release();
			instance = nullptr;
...
void LexState::SetLexer(uptr_t wParam) {
	...
	if (lexLanguage == SCLEX_CONTAINER) {
		...
		const char* ext=LexerNameFromID(lexLanguage); // 初始化
...
void LexState::SetLexerLanguage(const char* languageName) {
	const Lexilla::LexerModule* lex = Lexilla::catalogueLexilla.Find(languageName);
	if (!lex)
		lex = Lexilla::catalogueLexilla.Find(SCLEX_NULL);
	if (lex)
...
sptr_t ScintillaBase::WndProc(Message iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case Message::CallTipShow:
		//=================inc-new-flag=================
		ct.posWordStart = static_cast<Sci_Position>(wParam);
		//=================inc-new-end=================
