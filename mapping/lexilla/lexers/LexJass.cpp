...
static int GetNextVisibleCharP(StyleContext &sc, Sci_Position* pos) {
...
static int LongDelimCheck(StyleContext &sc,Sci_Position* pos) {
	if (!pos)
		return 0;
	int sep = 1;
	*pos = 1;
	GetNextVisibleCharP(sc,pos);
	while (sc.GetRelative(*pos) == '=' && sep < 0xFF){
		sep++;
		*pos = *pos + 1;
	}
	if (GetNextVisibleCharP(sc, pos) == sc.ch)
		return sep;
	return 0;
}
...
static void ColouriseJassLuaDoc(
	...
	int C1sepCount = 0;
	{
		...
		if (initStyle ==  SCE_JASS_LUA_LITERALSTRING || initStyle ==  SCE_JASS_LUA_COMMENT ||
			initStyle ==  SCE_JASS_LUA_STRING || initStyle ==  SCE_JASS_LUA_CHARACTER) {
	...
	int idenStyle =  SCE_JASS_LUA_IDENTIFIER;
	Sci_Position idenDimPos = 0;
	bool foundGoto = false;
	std::vector<std::string> wordlist;
	bool idenSetDefaultState = false;
	Sci_Position locpos = 0;
	// Do not leak onto next line
	if (initStyle ==  SCE_JASS_LUA_STRINGEOL || initStyle ==  SCE_JASS_LUA_COMMENTLINE || initStyle ==  SCE_JASS_LUA_PREPROCESSOR) {
		initStyle =  SCE_JASS_LUA_DEFAULT;
	}
	...
	for (; sc.More(); sc.Forward()) {
		if (sc.atLineEnd) {
			...
			switch (sc.state) {
			case  SCE_JASS_LUA_LITERALSTRING:
			case  SCE_JASS_LUA_COMMENT:
			case  SCE_JASS_LUA_STRING:
			case  SCE_JASS_LUA_CHARACTER:
		...
		if (sc.atLineStart && (sc.state ==  SCE_JASS_LUA_STRING)) {
			// Prevent  SCE_JASS_LUA_STRINGEOL from leaking back to previous line
			sc.SetState( SCE_JASS_LUA_STRING);
		}
		if (sc.atLineStart && (sc.state ==  SCE_JASS_LUA_COMMENTLINE)) {
			sc.SetState( SCE_JASS_LUA_DEFAULT);
		}
		// Handle string line continuation
		if ((sc.state ==  SCE_JASS_LUA_STRING || sc.state ==  SCE_JASS_LUA_CHARACTER) &&
			sc.ch == '\\') {
			if (sc.chNext == '\n' || sc.chNext == '\r') {
				sc.Forward();
		...
		if (idenSetDefaultState) {
			sc.SetState( SCE_JASS_LUA_DEFAULT);
			idenSetDefaultState = false;
		}
		if (C1sepCount>0 && nestLevel==0 
			&& (sc.state !=  SCE_JASS_LUA_STRING && sc.state !=  SCE_JASS_LUA_CHARACTER 
			&& sc.state !=  SCE_JASS_LUA_COMMENTLINE && sc.state !=  SCE_JASS_LUA_PREPROCESSOR)) {
			//不能判断注释或字符串类型。
			if (sc.ch == ']') {
				//jass_lua 自动离开范围,无多层级要求。
				int sep = LongDelimCheck(sc, &locpos);
				if (sep == 1 /* && C1sepCount == 1 */) {    // un-nest with ]]-only
					sc.SetState( SCE_JASS_LUA_IMPORT_FALG);
					sc.Forward(locpos);
					sc.ForwardSetState( SCE_JASS_LUA_DEFAULT);
					C1sepCount = 0;
				}
		...
		if (sc.state ==  SCE_JASS_LUA_OPERATOR) {
				...
				if (setWordStart.Contains(sc.GetRelative(ln))) {
					...
					if (!keywords.InList(s)) {
						...
						if (sc.GetRelative(ln) == ':' && sc.GetRelative(ln + 1) == ':') {
							...
							sc.ChangeState( SCE_JASS_LUA_LABEL);
							if (ws1) {
								sc.SetState( SCE_JASS_LUA_DEFAULT);
								sc.ForwardBytes(ws1);
							}
							sc.SetState( SCE_JASS_LUA_LABEL);
		...
		else if (sc.state ==  SCE_JASS_LUA_NUMBER) {
		...
		else if (sc.state ==  SCE_JASS_LUA_IDENTIFIER) {
			...
			if (idenWordPos > 0) {
				...
				idenPos -= idenWordPos;
				if (idenPos > 0) {
					sc.SetState( SCE_JASS_LUA_IDENTIFIER);
			...
			if (foundGoto) {					// goto <label> forward scan
				...
				if (setWordStart.Contains(sc.ch)) {
					...
					if (keywords.InList(s))		// labels cannot be keywords
						sc.ChangeState( SCE_JASS_LUA_WORD);
				}
				sc.SetState( SCE_JASS_LUA_DEFAULT);
			}
		}
		else if (sc.state ==  SCE_JASS_LUA_COMMENTLINE || sc.state ==  SCE_JASS_LUA_PREPROCESSOR) {
			if (sc.atLineEnd) {
				sc.ForwardSetState( SCE_JASS_LUA_DEFAULT);
			}
			//lua end
			else if (sc.state ==  SCE_JASS_LUA_COMMENTLINE && sc.ch == '?' && sc.chNext == '>')
			{
				sc.SetState( SCE_JASS_LUA_OPERATOR);
				sc.ForwardSetState( SCE_JASS_LUA_OPERATOR);
				//跳回jass
				sc.SetState(SCE_JASS_DEFAULT);
				return;
			}
		}
		else if (sc.state ==  SCE_JASS_LUA_STRING) {
			if (stringWs) {
				if (!IsASpace(sc.ch))
					stringWs = 0;
			}
			if (sc.ch == '\\') {
				...
				sc.ChangeState( SCE_JASS_LUA_STRINGEOL);
				sc.ForwardSetState( SCE_JASS_LUA_DEFAULT);
			}
		}
		else if (sc.state ==  SCE_JASS_LUA_CHARACTER) {
			if (stringWs) {
				if (!IsASpace(sc.ch))
					stringWs = 0;
			}
		...
		else if (sc.state ==  SCE_JASS_LUA_LITERALSTRING || sc.state ==  SCE_JASS_LUA_COMMENT) {
			if (sc.ch == '[') {
				const int sep = LongDelimCheck(sc,&locpos);
		...
		if (sc.state ==  SCE_JASS_LUA_DEFAULT) {
			if (IsADigit(sc.ch) || (sc.ch == '.' && IsADigit(sc.chNext))) {
				sc.SetState( SCE_JASS_LUA_NUMBER);
			...
			else if (sc.ch == '?' && sc.chNext == '>')
			{
				...
				idenWordPos = 0;
				idenDimPos = 0;
				idenStyle =  SCE_JASS_LUA_IDENTIFIER;
				foundGoto = false;
				int cNext;
				int locDimPos = 0;
				do {
					...
					ident += identSeg;
					const char* s = identSeg.c_str();

					wordlist.push_back(identSeg);

					int newStyle =  SCE_JASS_LUA_IDENTIFIER;
					if (keywords.InList(s)) {
						newStyle =  SCE_JASS_LUA_WORD;
					}

					//跳过分界符以支持cj.Atan2、...

					if (newStyle !=  SCE_JASS_LUA_IDENTIFIER) {
						idenStyle = newStyle;
						idenWordPos = idenPos;
					}
					if (idenStyle ==  SCE_JASS_LUA_WORD)	// keywords cannot mix
						break;
					cNext = sc.GetRelative(idenPos + 1);
					if ((c == '.' || c == ':') && setWordStart.Contains(cNext)) {
						ident += static_cast<char>(c);
						wordlist.push_back(std::string("") + static_cast<char>(c));
						idenPos++;
						locDimPos = idenPos;
					}
					else {
				...
				for (int i = 0; i < wordlist.size(); i++) {
					s = wordlist[i].c_str();
					if (s[0] == ':') {
						sc.SetState( SCE_JASS_LUA_WORD2);
					}
					// . or other
					else if (s[0] == '.') {
						sc.SetState( SCE_JASS_LUA_OPERATOR);
					}
					else if (idenStyle ==  SCE_JASS_LUA_WORD) {
						...
						sc.SetState( SCE_JASS_LUA_WORD5);
					}
					else if (keywords6.InList(s)) {
						sc.SetState( SCE_JASS_LUA_WORD6);
					}
					else if (keywords7.InList(s)) {
						sc.SetState( SCE_JASS_LUA_WORD7);
					}
					else {
						if (i == 0 && (!(sc.GetRelative(-2) !='.' && sc.chPrev=='.') && sc.chPrev != ':')) {
							if (strncmp("local", s, 5) == 0
								|| strncmp("self", s, 4) == 0
								|| strncmp("nil", s, 3) == 0
								|| strncmp("true", s, 4) == 0
								|| strncmp("flase", s, 5) == 0) {
							...
							else if (setLuaNameRight.Contains(GetNextVisibleChar(sc, wordlist[i].size()))) {
								sc.SetState( SCE_JASS_LUA_NAME);
							}
							else {
								sc.SetState( SCE_JASS_LUA_IDENTIFIER);
								break;
							...
							if (setLuaNameRight.Contains(GetNextVisibleChar(sc, wordlist[i].size()))) {
								sc.SetState( SCE_JASS_LUA_KEY);
							}
							else {
								sc.SetState( SCE_JASS_LUA_DEFAULT);
							}
					...
					if (i + 1 == wordlist.size()) {
						sc.Forward(wordlist[i].size()-1);
					}
					else {
						sc.Forward(wordlist[i].size());
					}
				}
				if (sc.state !=  SCE_JASS_LUA_IDENTIFIER) {
					idenPos = 0;
					idenStyle = 0;
					idenWordPos = 0;
					idenSetDefaultState = true;
				}
				else {
					sc.SetState( SCE_JASS_LUA_IDENTIFIER);
				}
				...
				sc.SetState( SCE_JASS_LUA_CHARACTER);
				stringWs = 0;
			}
			else if (sc.ch == '[') {
				sepCount = LongDelimCheck(sc,&locpos);
				if (sepCount == 0) {
					sc.SetState( SCE_JASS_LUA_OPERATOR);
				}
				else {
					...
					if (!is_lua_import) {
						nestLevel = 1;
						sc.SetState( SCE_JASS_LUA_LITERALSTRING);
						sc.Forward(locpos);
					}
				}
			}
			else if (sc.Match('-', '-')) {
				sc.SetState( SCE_JASS_LUA_COMMENTLINE);
				if (sc.Match("--[")) {
					sc.Forward(2);
					sepCount = LongDelimCheck(sc,&locpos);
					if (sepCount > 0) {
						...
						if (!C1sepCount) {
							...
							if (is_lua_import) {
								nestLevel = 0;
								sc.ChangeState( SCE_JASS_LUA_IMPORT_FALG);
								sc.Forward(locpos);
								C1sepCount = sepCount;
								//sc.ForwardSetState( SCE_JASS_LUA_DEFAULT);
							}
						...
						if(!is_lua_import) {
							nestLevel = 1;
							sc.ChangeState( SCE_JASS_LUA_COMMENT);
							sc.Forward(locpos);
						}
					}
				}
				...
				sc.SetState( SCE_JASS_LUA_PREPROCESSOR);	// Obsolete since Lua 4.0, but still in old code
			}
...
static void ColouriseJassDoc(
	...
	if (initStyle >=  SCE_JASS_LUA_DEFAULT) {
	...
	for (; sc.More(); sc.Forward()) {
		...
		if (sc.state == SCE_JASS_DEFAULT) {
			...
			//lua start
				...
				sc.Forward();//skip+2
				sc.SetState( SCE_JASS_LUA_DEFAULT);
				ColouriseJassLuaDoc(sc,  SCE_JASS_LUA_DEFAULT, keywordlists,styler);
			}
			else if (setWordStart.Contains(sc.ch)) {
				// For matching various identifiers with dots and colons, multiple
				// matches are done as identifier segments are added. Longest match is
...
static void FoldJassDoc(Sci_PositionU startPos, Sci_Position length, int /* initStyle */, WordList *[],
	...
	for (Sci_PositionU i = startPos; i < lengthDoc; i++) {
		...
		case  SCE_JASS_LUA_WORD:
			...
			continue;
			break;
		case  SCE_JASS_LUA_WORD5:
		case  SCE_JASS_LUA_WORD6:
		case  SCE_JASS_LUA_WORD7:
			break;
		case  SCE_JASS_LUA_OPERATOR:
			if (ch == '{') {
				levelCurrent++;
			}
			else if (ch == '}') {
...
namespace {
	...
	LexicalClass lexicalClasses[] = {
		// Lexer Lua SCLEX_LUA  SCE_JASS_LUA_:
		...
		19, " SCE_JASS_LUA_DEFAULT", "default", "White space: Visible only in View Whitespace mode (or if it has a back colour)",
		20, " SCE_JASS_LUA_COMMENT", "comment", "Block comment (Lua 5.0)",
		21, " SCE_JASS_LUA_COMMENTLINE", "comment line", "Line comment",
		22, " SCE_JASS_LUA_COMMENTDOC", "comment documentation", "Doc comment -- Not used in Lua (yet?)",
		23, " SCE_JASS_LUA_NUMBER", "literal numeric", "Number",
		24, " SCE_JASS_LUA_WORD", "keyword", "Keyword",
		25, " SCE_JASS_LUA_STRING", "literal string", "(Double quoted) String",
		26, " SCE_JASS_LUA_CHARACTER", "literal string character", "Character (Single quoted string)",
		27, " SCE_JASS_LUA_LITERALSTRING", "literal string", "Literal string",
		28, " SCE_JASS_LUA_PREPROCESSOR", "preprocessor", "Preprocessor (obsolete in Lua 4.0 and up)",
		29, " SCE_JASS_LUA_OPERATOR", "operator", "Operators",
		30, " SCE_JASS_LUA_IDENTIFIER", "identifier", "Identifier (everything else...)",
		31, " SCE_JASS_LUA_STRINGEOL", "error literal string", "End of line where string is not closed",
		32, " SCE_JASS_LUA_LABEL", "label", "Labels",
		33, " SCE_JASS_LUA_IMPORT_FALG", "luaimportflag", "luaimportflag",
		34, " SCE_JASS_LUA_NAME", "lua_name1", "lua_name1",
		35, " SCE_JASS_LUA_KEY", "lua_key1", "lua_key1",
		36, " SCE_JASS_LUA_WORD2", "identifier", "Other keywords",
		37, " SCE_JASS_LUA_WORD3", "identifier", "Other keywords",
		38, " SCE_JASS_LUA_WORD4", "identifier", "Other keywords",
		39, " SCE_JASS_LUA_WORD5", "identifier", "Other keywords",
		40, " SCE_JASS_LUA_WORD6", "identifier", "Other keywords",
		41, " SCE_JASS_LUA_WORD7", "identifier", "Other keywords",
	};
